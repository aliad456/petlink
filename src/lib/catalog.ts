...
export type CatalogCategory = { id: string; slug: string; name: string; icon: string | null; is_emergency: boolean; is_adoption: boolean };
...
export const getCatalog = unstable_cache(
  async () => {
    ...
    const [categories, filters, cities] = await Promise.all([
      supabase.from("categories").select("id, slug, name, icon, is_emergency, is_adoption").eq("is_visible", true).order("sort_order"),
      supabase
        .from("filters")
