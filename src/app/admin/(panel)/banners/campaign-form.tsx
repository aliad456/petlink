...
export function CampaignForm(props: CampaignFormProps) {
  ...
  const previewSrc = form.image_path
    ? `/preview-frame/ad/draft?${new URLSearchParams({
        p: form.placement,
        kind: form.kind,
        img: form.image_path,
        mimg: form.mobile_image_path,
        alt: form.alt_text,
      }).toString()}`
    : null;
  ...
  return (
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,26rem)]">
      {/* תצוגה מקדימה חיה: העמוד האמיתי עם המודעה */}
      <aside className="glass-lite flex flex-col gap-3 rounded-[1.75rem] p-4 lg:sticky lg:top-6 lg:order-last">
        <h2 className="text-center text-lg font-bold">תצוגה מקדימה</h2>
        {previewSrc ? (
          <DevicePreview
            key={`${form.placement}-${form.image_path}-${form.mobile_image_path}-${form.kind}`}
            src={previewSrc}
            maxHeight={640}
          />
        ) : (
          <p className="py-10 text-center text-sm text-muted">העלו תמונה כדי לראות איך המודעה תיראה באתר.</p>
        )}
      </aside>
